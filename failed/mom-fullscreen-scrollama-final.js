// Final Scrollama integration for activity groups

// Create scroll sections for each activity group
...
function setupEnhancedScrolling() {
    ...
    window.addEventListener('scroll', () => {
        ...
        const timelineEnd = introHeight + (activityGroups.length * windowHeight); // Use activity groups
        ...
        } else if (scrollTop >= gridSectionEnd) {
            // We're past the grid section - ensure grid stays visible
            if (!gridCreated) {
...
function addProgressIndicator() {
    ...
    window.updateProgressIndicator = function(currentIndex) {
        dots.style("background-color", (d, i) => i === currentIndex ? "#869F77" : "#e9ecef");
    };
}
...
function start() {
    if (!window.activityGroups || !window.activityGroups.length) return;
    createScrollSections();
    addProgressIndicator();
    setupEnhancedScrolling();
    setupSmoothNavigation();
    console.log("Final scrollama initialized with activity groups");
}

// Run when data is ready, or immediately if already available
if (window.activityGroups && window.activityGroups.length) {
    start();
} else {
    window.addEventListener('data-ready', start, { once: true });
}
